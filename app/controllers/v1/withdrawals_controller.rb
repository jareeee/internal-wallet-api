  ...
  class WithdrawalsController < ApplicationController
    ...
    def create
      ...
      ActiveRecord::Base.transaction do
        source_wallet = Wallet.lock.find(source_wallet_id)
      ...
      return render json: { error: "You are not authorized to withdraw from this wallet" }, status: :forbidden if unauthorized
      return render json: { error: "Insufficient funds" }, status: :unprocessable_entity if insufficient

      render json: serialize_transaction(withdrawal), status: :created
